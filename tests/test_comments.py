...
def test_pinned_coordinates_are_clamped_to_the_image(qapp):
    sketch = Sketch()
    comment = sketch.add_comment(10, "outside", x=-2, y=3)
    assert (comment["x"], comment["y"]) == (0.0, 1.0)
...
def test_pin_number_matches_its_position_in_the_full_comment_list(qapp):
    sketch = Sketch()
    sketch.set_frame(1)
    sketch.add_comment(1, "frame-level first")
    sketch.add_comment(1, "pinned second", x=0.5, y=0.5)

    image = _render(sketch)
    # The exact glyph is platform-font dependent; a non-background centre and
    # filled surround prove that the second list row produced a visible pin.
    assert probe_pixel(image, 100, 100)[:3] != (0, 0, 0)
...
def test_deserialize_comments_ignores_malformed_content(qapp):
    sketch = Sketch()
    sketch.deserialize_comments(
        {
            "bad-frame": [],
            "2": "not-a-list",
            "3": [None, "bad", {"text": "kept", "id": "ok"}],
        }
    )
    assert sketch.get_comments(3) == [{"text": "kept", "id": "ok"}]
